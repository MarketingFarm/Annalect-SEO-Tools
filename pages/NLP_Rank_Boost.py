...
if st.session_state.get('analysis_started', False):
    ...
    if ai_overview:
        ...
        col_text, col_sources = st.columns([2, 1.2])
        ...
        with col_sources:
            ...
            for ref in references:
                ...
                image_html = f'<div style="flex: 1; min-width: 120px;"><a href="{ref.get("url")}" target="_blank"><img src="{image_url}" style="width: 100%; border-radius: 8px;"></a></div>' if image_url else ''
                
                card_html = f"""
                <div style="background-color: #f0f4ff; border-radius: 12px; padding: 16px; margin-bottom: 1rem; display: flex; gap: 16px; align-items: center;">
                    <div style="flex: 3;">
