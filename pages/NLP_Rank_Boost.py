...
<style>
    ...
    h2, h3 { color: #555; }
    .aio-header h2 { border: none; margin: 0; padding: 0; color: #333; font-size: 24px; }
</style>
...
if st.session_state.get('analysis_started', False):
    ...
    if ai_overview:
        ...
        if references:
            ...
            cols = st.columns(2) # Crea due colonne per le fonti
            for i, ref in enumerate(references):
                with cols[i % 2]:
                    card_html = f"""
                    <div style="background-color: #e5edff; border-radius: 16px; padding: 16px; height: 100%;">
                        <div style="font-weight: 500; color: #1a0dab; margin-bottom: 8px;">
                            <a href="{ref.get('url')}" target="_blank" style="text-decoration: none; color: inherit;">{ref.get('title')}</a>
                        </div>
                        <div style="font-size: 14px; color: #4d5156; margin-bottom: 12px;">
                            {ref.get('text', '')}
                        </div>
